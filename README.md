...
- ``[GET] /gamejams/minMembers/{:minMembers}`` - retrieve upcoming and on-going ranked game jams from itch.io. Requires a number to set the minimum member count; the API will only return game jams that exceed this value. </br> Returns:
```
{
  upcoming: [
    {
...
```
- ``[GET] /gamejam/details/:link`` - retrieve the details of the specified game jam. Must be supplied with the sub-directory string of the game jam link. </br> 
Returns:
```
{
  title:	"$5K GameName Game Jam"
...
[
  {
    ...
    content:	"I can do pretty much everything required in game dev besides the..."
    replies:	1
